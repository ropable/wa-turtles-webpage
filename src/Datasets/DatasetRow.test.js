...
describe("DatasetRow test suite", function() {
  const ds = {
  ...
  /* Functions */
  it("gets a spatial extent from valid dataset", () => {
    const wrapper = shallow(<DatasetRow dataset={ds} key={"test"} />);
    const geom = wrapper
      .instance()
      .getGeom(ds, wrapper.instance().props.defaultGeom);
    expect(geom.type).toBe("MultiPolygon");
  });

  it("gets a spatial extent from fallback if dataset has invalid geom", () => {
    const wrapper = shallow(<DatasetRow dataset={ds} key={"test"} />);
    const geom = wrapper
      .instance()
      .getGeom({ test: "dataset" }, wrapper.instance().props.defaultGeom);
    expect(geom.type).toBe("MultiPolygon");
  });
});
